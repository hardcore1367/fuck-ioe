...
In **satellite communication**, a signal is sent from a ground station up to an orbiting satellite, which then amplifies and retransmits it back down to another ground station. It covers very large geographic areas and is used for TV broadcasting, GPS, and intercontinental communication. The main drawback is **propagation delay**, especially with geostationary satellites (~270 ms round trip).
